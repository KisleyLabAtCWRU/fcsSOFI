...
%data file name
fname = 'dataset68'; % don't include ".mat" in file name

%diffusion coefficient parameters
pixelsize=100; %in nm; needed to accurately calculate D
...
row_index = 18; %pixel row index
column_index = 12; %pixel column index
row_index2 = 18;
column_index2 = 18;
...
for i=1:size(AC_logbin,1)
    ...
    % characteristic time
    tauD(i)=model_coefs(n(1)); % in in seconds THIS NEEDS TO BE CHANGED SHAWN
...
hsvmap(1:szmap1,1:szmap2,2)=norm_ca_AC;%srmap_hsv;% Shawn change
...
if plotfigures == 1 
    ...
    if type == 3 || type == 6
        ...
        sofiBinarized = imbinarize(srmap_hsv, 0.01);
...
%% Single Pixel Results
    ...
    if examplecf == 1
        ...
        hold on
        plot(x2,1./(1+(x2./td_stp)),'--b','LineWidth',2)
        set(gca,'xscale','log')
        xlabel('\tau')
        ylabel('G(\tau)')
...
if savethedata == 1 
    ...
    elseif type == 4
         save(strcat(fname,'_analyzed_brownian_norm_',date),'fit_curves','fit_parameters','SOFI','D_map','D_map_corrected','Rsquare_map','fcsSOFI','fcsSOFI_cmap',...
             'normDmap2log','normcoef','filtim','type','szmap1','szmap2');
    elseif type == 5
         save(strcat(fname,'_analyzed_brownian_2_paramemers_',date),'fit_curves','fit_parameters','SOFI','D_map','D_map_corrected','Rsquare_map','fcsSOFI','fcsSOFI_cmap',...
             'normDmap2log','normcoef','filtim','type','szmap1','szmap2'); 
    elseif type == 6
         save(strcat(fname,'_analyzed_anomalous_2_paramemers_',date),'fit_curves','fit_parameters','SOFI','D_map','D_map_corrected','Rsquare_map','fcsSOFI','fcsSOFI_cmap',...
             'normDmap2log','normcoef','filtim','type','szmap1','szmap2');
    end
