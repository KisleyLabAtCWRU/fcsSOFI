...
XC2Corrected - The diffusion corrected version of the cross correlation 
    values. Still under development. Made from ideas from probe diff paper
...
function [XC2, XC2_nodistance_factor, XC3, XC2Corrected, sigma,std_sigma] = crossSofi(data)
    ...
    % Create XC2 matrix from XC2 calculations
    ...
    XC2_nodistance_factor = zeros(2*(height-2)-1, 2*(width-2)-1);
    XC2_nodistance_factor(1:2:end, 1:2:end) = AC2; 
    XC2_nodistance_factor(2:2:end, 1:2:end) = XC2Vertical(1:end-1, :); 
    XC2_nodistance_factor(1:2:end, 2:2:end) = XC2Horizontal(:, 1:end-1);  
    XC2_nodistance_factor(2:2:end, 2:2:end) = XC2Diagonal(1:end-1, 1:end-1);
    ...
    % fprint(distVert(filter))
    std_Vert = std(distVert(filter),"omitnan");
    std_Hor = std(distHor(filter),"omitnan");
    std_Diag1 = std(distDiag1(filter),"omitnan");
    std_Diag2 = std(distDiag2(filter),"omitnan");
    ...
    std_X = (sqrt(-log(distHor))/(2*distHor))*(log(distHor))^2*std_Hor;
    std_Y = (sqrt(-log(distVert))/(2*distVert))*(log(distVert))^2*std_Vert;
    std_Diag1 = (sqrt(-log(distDiag1))/(2*distDiag1))*(log(distDiag1))^2*std_Diag1;
    std_Diag2 = (sqrt(-log(distDiag2))/(2*distDiag2))*(log(distDiag2))^2*std_Diag2;
    std_sigma = sqrt(1/16*(std_X^2 + std_Y^2 + std_Diag1^2 + std_Diag2^2));
